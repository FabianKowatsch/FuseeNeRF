...
namespace InstantNeRF
{
    public class GridSampler : nn.Module
    {
        ...
        private readonly int GRIDSIZE = 128;
        ...
        public GridSampler(DataProvider dataProvider, int updateGridFrequency = 16, float nearDistance = 0.2f, int densityActivation = 2, int rgbActivation = 3, int targetBatchSize = 1 << 18) : base("GridSampler")
        {
            ...
            // set up density grids and bitfield as permanent buffers
            ...
            // image metadata required for ray marching
        ...
        public void updateDensityGrid(MLP mlp)
        {
            ...
            {
                // For earlier iterations, take M uniform samples
                updateDensityGridBasedOnIteration(M, 0, mlp);
            }
            else
            {
                // For later iterations, take M/4 uniform samples and M/4 samples in occupied cells
        ...
        private void updateDensityGridBasedOnIteration(int nUniformSamples, int nNonUniformSamples, MLP mlp)
        {
            if (this.dataInfo != null)
            {
                ...
                // Sampled uniformly across all cells, depending on the passed density threshold
                (Tensor positionsUniform, Tensor indicesUniform) = RaymarchApi.generateGridSamples(
                    ...
                    dataInfo.aabbMin,
                    dataInfo.aabbMax);
                // Sampled across occupied cells, depending on the passed density threshold
                ...
                // Set the value of a cell in the floating point grid to the maximum of its current value and the sampled value at that position
                RaymarchApi.splatGridSamplesNerfMaxNearestNeighbour(density, indices, PADDED_OUTPUT_WIDTH, totalSamples, temporaryGrid);

                // Decay empty cells exponentially
                RaymarchApi.sampleDensityGridEma(temporaryGrid, nElementsDensity, EMA_DECAY, densityGrid);

                this.densityGrid.detach_();
                this.emaStep++;
                // Update the bitfield by comparing the corresponding value in the density grid with a threshold
        ...
        // Generate samples for training and inference
        public Dictionary<string, Tensor> Sample(Dictionary<string, Tensor> data, MLP mlp)
        {
            ...
            // For inference, samples are compacted during volume rendering
            if (!this.training)
            {
            ...
            if(!useCompaction)
            {
                data.Add("coords", coords.detach());
                data.Add("rayNumsteps", rayNumsteps.detach());
                data.Add("rayNumstepsCompacted", rayNumsteps.detach());
                data.Add("positions", coords.slice(-1, 0, 3, 1));
                data.Add("directions", coords.slice(-1, 4, coords.size(-1), 1));

                this.iteration++;
                return data;
            }
            // For training, compacted samples are generated by querying the density MLP and calculatimg the transmittance to skip occluded space
